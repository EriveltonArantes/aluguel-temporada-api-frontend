...
const apiBaseUrl = "https://aluguel-temporada-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Proprietario</li>
          <li>✓ Controle completo de Imovel</li>
          <li>✓ Controle completo de Reserva</li>
          <li>✓ Controle completo de Pagamento</li>
        </ul>
      </div>
...
function PainelProprietario({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "pix", "banco", "agencia", "conta", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/proprietarios", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/proprietarios/" + editId : apiBaseUrl + "/api/proprietarios";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "pix", "banco", "agencia", "conta"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/proprietarios/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Proprietario")}</div>
            ...
            <div className="item-field"><b>pix</b><span>{item["pix"] ?? "—"}</span></div>
            <div className="item-field"><b>banco</b><span>{item["banco"] ?? "—"}</span></div>
            <div className="item-field"><b>agencia</b><span>{item["agencia"] ?? "—"}</span></div>
            <div className="item-field"><b>conta</b><span>{item["conta"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Proprietario</h3>
      ...
      <label className="field-label">pix</label>
      ...
      <input type="text" value={form["agencia"] ?? ""} onChange={e => setForm({...form, agencia: e.target.value})} />
      <label className="field-label">conta</label>
      <input type="text" value={form["conta"] ?? ""} onChange={e => setForm({...form, conta: e.target.value})} />
              <div className="modal-actions">
...
function PainelImovel({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [proprietarioList, setProprietarioList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["proprietarioId", "nome", "endereco", "cidade", "estado", "tipo", "capacidade", "precoDiaria", "descricao", "ativo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/imovels", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/proprietarios", { headers: authHeaders(token) }).then(r => r.json()).then(setProprietarioList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/imovels/" + editId : apiBaseUrl + "/api/imovels";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["proprietarioId", "nome", "endereco", "cidade", "estado", "tipo", "capacidade", "precoDiaria", "descricao", "ativo"].forEach(k => { f[k] = item[k] ?? ""; });
    ["proprietario"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/imovels/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              <div className="item-photo-fallback">I</div>
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/imovel" + item.id + "/320/200"} alt="Imovel" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🏠</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["nome"] ?? "Imovel")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>proprietarioId</b><span>{item["proprietarioId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>proprietario</b><span>{item.proprietarioId ? ((proprietarioList.find(o => o.id === item.proprietarioId) || {}).nome ?? ("#" + item.proprietarioId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Imovel</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">proprietarioId</label>
      <input type="number" value={form["proprietarioId"] ?? ""} onChange={e => setForm({...form, proprietarioId: e.target.value})} />
      <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      ...
      <select value={form["proprietario"] ?? ""} onChange={e => setForm({...form, proprietario: e.target.value})}>
        <option value="">Selecione...</option>
        {(proprietarioList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelReserva({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const [imovelList, setImovelList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["imovelId", "nomeHospede", "emailHospede", "telefoneHospede", "dataEntrada", "dataSaida", "numHospedes", "valorTotal", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/imovels", { headers: authHeaders(token) }).then(r => r.json()).then(setImovelList).catch(() => {});
  }, [token]);
  ...
  const editar = (item) => {
    const f = {};
    ["imovelId", "nomeHospede", "emailHospede", "telefoneHospede", "dataEntrada", "dataSaida", "numHospedes", "valorTotal", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["imovel"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
    ...
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["nomeHospede"] ?? "Reserva")}</div>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              <div className="item-photo-fallback">R</div>
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/reserva" + item.id + "/320/200"} alt="Reserva" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🗂️</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["nomeHospede"] ?? "Reserva")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>imovelId</b><span>{item["imovelId"] ?? "—"}</span></div>
            <div className="item-field"><b>emailHospede</b><span>{item["emailHospede"] ?? "—"}</span></div>
            <div className="item-field"><b>telefoneHospede</b><span>{item["telefoneHospede"] ?? "—"}</span></div>
            <div className="item-field"><b>dataEntrada</b><span>{item["dataEntrada"] ? new Date(item["dataEntrada"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>dataSaida</b><span>{item["dataSaida"] ? new Date(item["dataSaida"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>numHospedes</b><span>{item["numHospedes"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>imovel</b><span>{item.imovelId ? ((imovelList.find(o => o.id === item.imovelId) || {}).nome ?? ("#" + item.imovelId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">imovelId</label>
      <input type="number" value={form["imovelId"] ?? ""} onChange={e => setForm({...form, imovelId: e.target.value})} />
      <label className="field-label">nomeHospede</label>
      <input type="text" value={form["nomeHospede"] ?? ""} onChange={e => setForm({...form, nomeHospede: e.target.value})} />
      <label className="field-label">emailHospede</label>
      <input type="text" value={form["emailHospede"] ?? ""} onChange={e => setForm({...form, emailHospede: e.target.value})} />
      <label className="field-label">telefoneHospede</label>
      <input type="text" maxLength={15} value={form["telefoneHospede"] ?? ""} onChange={e => setForm({...form, telefoneHospede: formatarTelefone(e.target.value)})} />
      <label className="field-label">dataEntrada</label>
      <input type="datetime-local" value={form["dataEntrada"] ?? ""} onChange={e => setForm({...form, dataEntrada: e.target.value})} />
      <label className="field-label">dataSaida</label>
      <input type="datetime-local" value={form["dataSaida"] ?? ""} onChange={e => setForm({...form, dataSaida: e.target.value})} />
      <label className="field-label">numHospedes</label>
      <input type="number" value={form["numHospedes"] ?? ""} onChange={e => setForm({...form, numHospedes: e.target.value})} />
      ...
      <label className="field-label">imovel</label>
      <select value={form["imovel"] ?? ""} onChange={e => setForm({...form, imovel: e.target.value})}>
        <option value="">Selecione...</option>
        {(imovelList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelPagamento({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["reservaId", "valor", "formaPagamento", "status", "dataHora", "comprovante", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/pagamentos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/pagamentos/" + editId : apiBaseUrl + "/api/pagamentos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["reservaId", "valor", "formaPagamento", "status", "dataHora", "comprovante"].forEach(k => { f[k] = item[k] ?? ""; });
    ["reserva"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/pagamentos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["formaPagamento"] ?? "Pagamento")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/pagamentos/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              ...
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/pagamento" + item.id + "/320/200"} alt="Pagamento" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">💳</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["formaPagamento"] ?? "Pagamento")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            ...
            <div className="item-field"><b>comprovante</b><span>{item["comprovante"] ?? "—"}</span></div>
            <div className="item-field"><b>reserva</b><span>{item.reservaId ? ((reservaList.find(o => o.id === item.reservaId) || {}).nomeHospede ?? ("#" + item.reservaId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Pagamento</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <input type="file" className="upload-input" onChange={e => uploadArquivo(e.target.files[0], v => setForm(f => ({...f, comprovante: v})), token)} />
      {form["comprovante"] && <a className="upload-preview-link" href={apiBaseUrl + form["comprovante"]} target="_blank" rel="noreferrer">Ver arquivo enviado</a>}
      <label className="field-label">reserva</label>
      <select value={form["reserva"] ?? ""} onChange={e => setForm({...form, reserva: e.target.value})}>
        <option value="">Selecione...</option>
        {(reservaList || []).map(o => (<option key={o.id} value={o.id}>{o.nomeHospede ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Proprietario");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Aluguel Temporada<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Proprietario" ? " active" : "")} onClick={() => setAba("Proprietario")}><span className="nav-ico">🗂️</span>Proprietario</button>
        <button className={"nav-btn" + (aba === "Imovel" ? " active" : "")} onClick={() => setAba("Imovel")}><span className="nav-ico">🏠</span>Imovel</button>
        <button className={"nav-btn" + (aba === "Reserva" ? " active" : "")} onClick={() => setAba("Reserva")}><span className="nav-ico">🗂️</span>Reserva</button>
        <button className={"nav-btn" + (aba === "Pagamento" ? " active" : "")} onClick={() => setAba("Pagamento")}><span className="nav-ico">💳</span>Pagamento</button>
      ...
      <main className="main">
        ...
        {aba === "Proprietario" && <PainelProprietario token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Imovel" && <PainelImovel token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Reserva" && <PainelReserva token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Pagamento" && <PainelPagamento token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
